...
#Scenario A, Baseline
parms <- c(L=0.000066, t=7, a=0.001, a1=1, a2=0.2, a3=1, a4=1, a5=0.2, a6=1, a7=1.5, d=0.5, D=0.01, D1=1, D2=1.5, D3=2, NS=100000, e=1.3, e1=0, e2=0, e3=2, e4=1, e5=2, e6=1, e7=2, e8=1, gamma=0.000018, R1=1, R2=0, R3=0, R4=0, R5=0, R6=0, SA=1, SB=0, SC=0)
...
for(i in 1:nsamples){
  ...
  store_out[i,] <- c(i,out[74,"ND"])
  }
store_out <- as.data.frame(store_out)
colnames(store_out) <- c("lhs_sample","output")
view(store_out)
save(store_out, file='LHS_ND_WithTreatmentDuration.Rdata')
...
view(mergedlhs)
save(store_out, file='LHS_merged_WithTreatmentDuration.Rdata')
#Partial Rank Correlation
install.packages("sensitivity")
library(sensitivity)
bonferroni.alpha <- 0.05/12
prcc <- pcc(mergedlhs[,3:10], mergedlhs[,2], nboot = 1000, rank=TRUE, conf=1-bonferroni.alpha)
save(prcc, file='prcc_mortality.Rdata')
summary <- print(prcc)

#Plotting
plot(summary$original, main='Partial rank correlation coefficients, Death', ylim=c(-1,1),
...
#LHS Code with Treatment Duration as a Constant
param_ranges <- as.data.frame(cbind(c(0.0005,0.01), c(0.25,0.99), c(0.005,0.02), 
                                    c(1.1,2), c(2,4), c(0.000001, 0.0001), c(1.1, 5)))
colnames(param_ranges) <- c("min","max")
get.size <- function(out) tail(1,out$ND)
nsamples <- 100
...
view(param_ranges)
Y[,1] <- 7
Y[,2] <- qunif(X[,1], min = param_ranges[1,1], max = param_ranges[2,1])
Y[,3] <- qunif(X[,2], min = param_ranges[1,2], max = param_ranges[2,2])
Y[,4] <- qunif(X[,3], min = param_ranges[1,3], max = param_ranges[2,3]) 
Y[,5] <- qunif(X[,4], min = param_ranges[1,4], max = param_ranges[2,4])
Y[,6] <- qunif(X[,5], min = param_ranges[1,5], max = param_ranges[2,5]) 
Y[,7] <- qunif(X[,6], min = param_ranges[1,6], max = param_ranges[2,6])
Y[,8] <- qunif(X[,7], min = param_ranges[1,7], max = param_ranges[2,7])
Y[,9] <- 0.000066
Y[,10] <- 1.375
Y[,11] <- 1.125
Y[,12] <- 1.5
...
colnames(store_out) <- c("lhs_sample","output")
view(store_out)
save(store_out, file='LHS_ND_WithoutTreatmentDuration.Rdata')
