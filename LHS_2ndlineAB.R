...
for(i in 1:nsamples){
  ...
  store_out[i,] <- c(i,out[74,"TCC"])
}
store_out <- as.data.frame(store_out)
colnames(store_out) <- c("lhs_sample","output")
view(store_out)
save(store_out, file='LHS_TCC_WithTreatmentDuration.Rdata')
...
view(mergedlhs)
save(store_out, file='LHS_2ndlineAB_merged_WithTreatmentDuration.Rdata')
#Partial Rank Correlation
install.packages("sensitivity")
library(sensitivity)
bonferroni.alpha <- 0.05/12
prcc <- pcc(mergedlhs[,3:10], mergedlhs[,2], nboot = 1000, rank=TRUE, conf=1-bonferroni.alpha)
save(prcc, file='prcc_2ndlineAB.Rdata')
summary <- print(prcc)

#Plotting
plot(summary$original, main='Partial rank correlation coefficients, second line antibiotic usage', ylim=c(-1,1),
