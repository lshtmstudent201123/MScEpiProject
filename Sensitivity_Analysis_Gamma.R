...
new.basic.model <- function(t, state, parms) { 
  with(as.list(c(state, parms)),{ 
    ...
    dNR = AESO*R1 + AESOEFO*R2 + AESOESO*R2 + AESOEFEO*R2 + AESOEFOEO*R2 + AESEFO*R3 + AESEFEO*R2 + AESEFOEO*R2 + AESESO*R4 + BEFO*R5 + BEFEO*R2 + BEFOEO*R2 + CESO*R3
    ...
    dTC = AESOEFE*d - AESOEFEO*(R2 + e3*D3) + AESOEFOE*d - AESOEFOEO*(R2 + e3*D3) + AESOES*d - AESOESO*(R2 + e2*D3) + AESEFE*d - AESEFEO*(R2 + e5*D3) + AESEFOE*d - AESEFOEO*(R2 + e5*D3) + AESES*d - AESESO*(R4 + e4*D3) + BEFE*d - BEFEO*(R2 + e7*D3) + BEFOE*d - BEFOEO*(R2 + e7*D3) + CES*d - CESO*(R3 + e8*D3) + SC*(NS*h*WA - AES*(e1*D1 + d +a1 +a2) + NS*h*WB - BEF*(a3 + d + e6*D2) + NS*h*WC - CES*(d + e8*D3))
...
install.packages("~/Desktop/Thesis/ODEsensitivity_1.1.2.tar.gz", repos = NULL, type="source")
install.packages("ODEnetwork")
library("ODEnetwork")
library("ODEsensitivity")

#Sensitivity
LVpars <- c("L", "t", "a1", "a2", "a3", "a4", "a5", "a6", "a7", "d", "D", "D1", "D2", "D3", "NS", "e", "e1", "e2", "e3", "e4", "e5", "e6", "e7", "e8", "gamma", "R1", "R2", "R3", "R4", "R5", "R6", "SA", "SB", "SC")
LVbinf <- c(L=0.000066, t=7, a1=0.001, a2=0.001/5, a3=0.001, a4=0.001, a5=0.001/5, a6=0.001, a7=0.001*1.5, d=0.5, D=0.01, D1=0.01, D2=0.01*1.5, D3=0.01*2, NS=100000, e=1.3^((1/0.5)/2), e1=(1.3^((1/0.5)/2))^0, e2=(1.3^((1/0.5)/2))^0, e3=(1.3^((1/0.5)/2))^2, e4=(1.3^((1/0.5)/2)), e5=(1.3^((1/0.5)/2))^2, e6=(1.3^((1/0.5)/2)), e7=(1.3^((1/0.5)/2))^2, e8=(1.3^((1/0.5)/2)), gamma=0.000018, R1=(1/(7-(1/0.5))), R2=1/7, R3=(1/7), R4=(1/7), R5=(1/7), R6=(1/7), SA=1, SB=0, SC=0)
LVbsup <- c(L=0.000066, t=7, a1=0.001, a2=0.001/5, a3=0.001, a4=0.001, a5=0.001/5, a6=0.001, a7=0.001*1.5, d=0.5, D=0.01, D1=0.01, D2=0.01*1.5, D3=0.01*2, NS=100000, e=1.3^((1/0.5)/2), e1=(1.3^((1/0.5)/2))^0, e2=(1.3^((1/0.5)/2))^0, e3=(1.3^((1/0.5)/2))^2, e4=(1.3^((1/0.5)/2)), e5=(1.3^((1/0.5)/2))^2, e6=(1.3^((1/0.5)/2)), e7=(1.3^((1/0.5)/2))^2, e8=(1.3^((1/0.5)/2)), gamma=0.0001, R1=(1/(7-(1/0.5))), R2=1/7, R3=(1/7), R4=(1/7), R5=(1/7), R6=(1/7), SA=1, SB=0, SC=0)
LVinit <- c(h=0.00000212, TAA=0, TBB=0, TCC=0, ND=0, NR=0, TA=0, TB=0, TC=0, NE=0, WA= 0.914, WB=0.036, WC=0.050, AES= 0, AESD= 0, AESO= 0, AESOR= 0, AESOD= 0, AESOEF= 0, AESOES= 0, AESOEFE= 0, AESOEFO= 0, AESOEFD= 0, AESOESD= 0, AESOESO= 0, AESOEFED= 0, AESOEFEO= 0, AESOEFOR= 0, AESOEFOD= 0, AESOEFOE= 0, AESOESOR= 0, AESOESOD= 0, AESOEFEOR= 0, AESOEFEOD= 0, AESOEFOED= 0, AESOEFOEO= 0, AESOEFOEOR= 0, AESOEFOEOD= 0, AESEF= 0, AESEFE= 0, AESEFO= 0, AESEFD= 0, AESEFED= 0, AESEFEO= 0, AESEFOR= 0, AESEFOD= 0, AESEFOE= 0, AESEFOEO= 0, AESEFEOR= 0, AESEFEOD= 0, AESEFOED= 0, AESEFOEOR= 0, AESEFOEOD= 0, AESES= 0, AESESD= 0, AESESO= 0, AESESOR= 0, AESESOD= 0, BEF= 0, BEFE= 0, BEFO= 0, BEFD= 0, BEFED= 0, BEFEO= 0, BEFOR= 0, BEFOD= 0, BEFOE= 0, BEFEOR= 0, BEFEOD= 0, BEFOED= 0, BEFOEO= 0, BEFOEOR= 0, BEFOEOD= 0, CES= 0, CESD= 0, CESO= 0, CESOR= 0, CESOD= 0)
LVtimes <- seq(from=365,to=365*5, by=365)
set.seed(59281)
...
# }
# NOT RUN {
ScenA_Gamma <- ODEsobol(mod = new.basic.model,
                        pars = LVpars,
                        state_init = LVinit,
                        times = LVtimes,
                        n = 100,
